...
def svn_history():
    ...
    print pSID, path_info[0][1], path_info[0][2], path_info[0][3]
    ...
    tmpQuery = ' order by s_revision desc limit ' + str(pOffset) + ',10'

    if pSvnId:
        query = 'select * from svn_history where s_path_id=? and s_id LIKE ?' + tmpQuery
        cur = g.db.execute(query, [path_info[0][0], '%' + pSvnId + '%'])
    else:
        query = 'select * from svn_history where s_path_id=?' + tmpQuery
